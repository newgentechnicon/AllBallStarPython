...
export default function CreateProductPage() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            if (user) {
                const { data: farmData, error: farmError } = await supabase.from('farms').select('id').eq('user_id', user.id).single();
                if (farmError || !farmData) {
                    Toastify({ text: "Could not find your farm. Please create one first.", duration: 3000, style: { background: "#EF4444" } }).showToast();
                    router.push('/farm');
                    return;
                }
                ...
                if (morphError) {
                    console.error("Error fetching morphs:", morphError);
                } else {
                    setAllMorphs(morphData || []);
                }
            }
        };
        fetchData();
    ...
    // --- FIX: Create a helper function to clear errors ---
    ...
    const handleAddMorph = (morph: Morph) => {
        if (!selectedMorphs.find(m => m.id === morph.id)) {
            ...
            clearError('morphs'); // Use the helper function
        }
    };
    ...
    const handlePictureChange = (e: ChangeEvent<HTMLInputElement>) => {
        if (e.target.files) {
            ...
            clearError('pictures'); // Use the helper function
    ...
    const validateForm = (): boolean => {
        ...
        if (!priceRaw.trim()) newErrors.price = 'Product price is required.';
        if (selectedMorphs.length === 0) newErrors.morphs = 'Morph is required.';
        if (!sex) newErrors.sex = 'Sex is required.';
        if (!year) newErrors.year = 'Year is required.';
        if (!description.trim()) newErrors.description = 'Product Description is required.';
        if (pictures.length === 0) newErrors.pictures = 'Product Picture is required at least one.';
    ...
    const handleSubmit = async (e: FormEvent) => {
        e.preventDefault();
        if (!validateForm() || !user || !farmId) return;
        ...
        try {
            ...
            const { data: newProduct, error: insertError } = await supabase.from('products').insert({
                farm_id: farmId, user_id: user.id, name: productName,
                price: parseFloat(priceRaw) || null, sex, year, description,
            ...
            const morphsToInsert = selectedMorphs.map(morph => ({ product_id: newProduct.id, morph_id: morph.id }));
            ...
            Toastify({ text: "Product added successfully!", duration: 2000, style: { background: "#14B8A6", borderRadius: "8px" } }).showToast();
            setTimeout(() => {
    ...
    const filteredCategories = useMemo(() => {
        ...
        for (const cat of allMorphs) {
            const matchedMorphs = (cat.morphs ?? []).filter(m => (!text || m.name.toLowerCase().includes(text)) && !selectedMorphs.find(sm => sm.id === m.id));
            const matchedSubs = (cat.sub_categories ?? []).map(sub => ({...sub, morphs: (sub.morphs ?? []).filter(m => (!text || m.name.toLowerCase().includes(text)) && !selectedMorphs.find(sm => sm.id === m.id))})).filter(sub => sub.morphs.length > 0);
    ...
    const handlePriceChange = (e: ChangeEvent<HTMLInputElement>) => {
        ...
        clearError('price'); // Use the helper function
    };
            ...
            <div className="container mx-auto max-w-lg py-4">
                ...
                <form onSubmit={handleSubmit} className="p-4 space-y-6" noValidate>
                    <div>
                        ...
                        <input type="text" id="productName" value={productName} onChange={e => { setProductName(e.target.value); clearError('productName'); }} className={`w-full rounded-lg ${errors.productName ? 'border-red-500' : 'border-gray-300'}`} placeholder="Put product name here" />
                    ...
                    <div>
                        ...
                        <input type="text" id="price" value={priceFormatted} onChange={handlePriceChange} className={`w-full rounded-lg ${errors.price ? 'border-red-500' : 'border-gray-300'}`} placeholder="Put product price here" inputMode="numeric" />
                        {errors.price && <p className="mt-1 text-sm text-red-600">{errors.price}</p>}
                    </div>

                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">Morph*</label>
                        <div className={`flex flex-wrap gap-2 rounded-lg border p-2 min-h-[4rem] ${errors.morphs ? 'border-red-500' : 'border-gray-300'}`}>
                        ...
                        {errors.morphs && <p className="mt-1 text-sm text-red-600">{errors.morphs}</p>}
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <select id="sex" value={sex} onChange={e => { setSex(e.target.value); clearError('sex'); }} className={`w-full rounded-lg ${errors.sex ? 'border-red-500' : 'border-gray-300'}`} >
                        ...
                        <div>
                            ...
                            <select id="year" value={year} onChange={e => { setYear(e.target.value); clearError('year'); }} className={`w-full rounded-lg ${errors.year ? 'border-red-500' : 'border-gray-300'}`} >
                                <option value="">Select</option>
                    ...
                    <div>
                        ...
                        <textarea id="description" value={description} onChange={e => { setDescription(e.target.value); clearError('description'); }} rows={4} maxLength={100} className={`w-full rounded-lg ${errors.description ? 'border-red-500' : 'border-gray-300'}`} placeholder="Put product description here"></textarea>
                        {errors.description && <p className="mt-1 text-sm text-red-600">{errors.description}</p>}
                        <p className="text-right text-xs text-gray-500">{description.length}/100</p>
                    </div>

                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">Product Picture* ({pictures.length}/3)</label>
                        <div className={`mt-1 flex justify-center rounded-md border-2 border-dashed px-6 pt-5 pb-6 ${errors.pictures ? 'border-red-500' : 'border-gray-300'}`}>
                            <div className="space-y-1 text-center">
                                ...
                                <input id="picture-upload" type="file" className="sr-only" multiple onChange={handlePictureChange} accept="image/*" disabled={pictures.length >= 3} />
