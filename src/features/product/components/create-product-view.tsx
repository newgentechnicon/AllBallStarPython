...
type MorphCategory = Tables<'morph_categories'> & { morphs: Morph[]; sub_categories: MorphSubCategory[] };
...
export function CreateProductView({ allMorphs }: CreateProductViewProps) {
    ...
    const handleFileRemoved = (indexToRemove: number) => {
        ...
        // ถ้าไฟล์ที่ลบเป็นไฟล์ใหม่ (blob) ให้ลบออกจาก state `pictures` ด้วย
        if (removedPreview.startsWith('blob:')) {
            const blobIndexToRemove = picturePreviews.slice(0, indexToRemove).filter(p => p.startsWith('blob:')).length;
            setPictures(currentFiles => currentFiles.filter((_, i) => i !== blobIndexToRemove));
        }
    };
            ...
            <div className="container mx-auto max-w-lg py-4">
                ...
                <form action={formActionWithFiles} className="p-4 space-y-6" noValidate>
                    <div>
                        ...
                        <input type="text" id="name" name="name" className={inputClassName(!!state.errors.name)} defaultValue={state.fields?.name as string} />
                ...
                onFilesAdded={(newFiles) => setPictures(prev => [...prev, ...newFiles])}
                onFileRemoved={handleFileRemoved}
                maxFiles={3}
                // error={state.errors.images?.[0]} // ถ้าใช้ useActionState
            />

                    <div>
                        <label htmlFor="price" className="block text-sm font-medium text-gray-700 mb-1">Product Price*</label>
                        <input type="number" id="price" name="price" className={inputClassName(!!state.errors.price)} defaultValue={state.fields?.price as string} inputMode="numeric" />
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <select id="sex" name="sex" className={inputClassName(!!state.errors.sex)} defaultValue={state.fields?.sex as string} >
                        ...
                        <div>
                            ...
                            <select id="year" name="year" className={inputClassName(!!state.errors.year)} defaultValue={state.fields?.year as string} >
                                <option value="">Select</option>
                    ...
                    <div>
                        ...
                        <textarea id="description" name="description" rows={4} className={inputClassName(!!state.errors.description)} defaultValue={state.fields?.description as string}></textarea>
